...
.. c:function:: int sasl_auxprop_request(sasl_conn_t *conn, const char ** propnames)
    ...
    **sasl_auxprop_request** will request that the SASL library
    ...
    :param propnames:  a NULL-terminated array of property names to request.  Note that this array must persist until a call to :saslman:`sasl_dispose(3)` on the :c:type:`sasl_conn_t`.

    :returns: Returns  :c:macro:`:c:macro:`SASL_OK`` on success. See :saslman:`sasl_errors(3)` for meanings of other return codes.
